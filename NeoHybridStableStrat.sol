...
import "./interfaces/INeoHybridMultiStrat.sol";
...
contract NeoHybridStableStrat is INeoHybridMultiStrat, Ownable, Pausable {
    ...
    // NeoHybrid Farm address.
    address public NeoHybridFarmAddress;
    ...
    modifier onlyFarms() {
        require(
            msg.sender == NeoHybridFarmAddress,
            "NeoHybridMultiStrat::onlyFarms: Caller is not a farms"
        );
        _;
    }
    ...
    modifier onlyGovernance() {
        require(
            ...
            "NeoHybridMultiStrat::onlyGovernance: Not gov"
        );
        _;
    }
    ...
    constructor(
        ...
        NeoHybridFarmAddress = _NeoHybridFarmAddress;
    ...
    function init(
        ...
        require(!isInit, "NeoHybridStableStrat::init: Already init");
    ...
    function balances(address user) external view override returns (uint256) {
        return INeoHybridStrat(stratAddress).getUserWant(user);
    }
        ...
        returns (uint256)
    {
        ...
        _wantAmt = INeoHybridStrat(stratAddress).deposit(user, _wantAmt);
    ...
    function withdraw(address user, uint256 _wantAmt, address _wantAddress)
        ...
        returns (uint256)
    {
        require(_wantAmt > 0, "NeoHybridMultiStrat::withdraw: Zero _wantAmt");
        _wantAmt = INeoHybridStrat(stratAddress).withdraw(user, _wantAmt);
        _unfarm(_wantAmt);
        ...
        IERC20(_wantAddress).safeTransfer(NeoHybridFarmAddress, _wantAmt);
        _collect();
        return _wantAmt;
    }
    ...
    function setDevAddress(address _devAddress)
        ...
        onlyGovernance
    {
        require(_devAddress != address(0), "NeoHybridMultiStrat::set: Zero address");
        devAddress = _devAddress;
    ...
    function updateWithdrawalMultiplier(uint256 _withdrawalMultiplier) external onlyGovernance {
        require(_withdrawalMultiplier <= DIVISOR, "NeoHybridMultiStrat::updateWithdrawalMultiplier: Multiplier > max");
        withdrawalMultiplier = _withdrawalMultiplier;
    }
